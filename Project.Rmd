...
major <- em %>% left_join(full_time, by = 'idCurrentPosition', copy = T) %>% filter(!is.na(salary)) %>% select(graduationId, count, salary, relatedToMajor) %>% group_by(count, relatedToMajor)
```
